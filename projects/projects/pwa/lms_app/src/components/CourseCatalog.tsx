    ...
    <article className="course-card course-card-hover-group">
      <Link to={`/cursos/${course.id}`} className="course-card-link">
        <div className="course-card-hero">
          <img src={course.image} alt={course.title} className="course-image" loading="lazy" decoding="async" />
    ...
    <div className="courses-grid" role="list">
      {courses.map((course) => (
        <div role="listitem" key={course.id}>
          <CourseCard course={course} progress={progress} enrolledCourses={enrolledCourses} />
        </div>
      ))}
    </div>
  );
