...
async function isCourseCompletedByUser(
  ...
  // T324: progression is also blocked if required evaluations have not been passed
  ...
  `;
  const gate = (gateRows[0] as DatabaseRow | undefined)?.gate as
    | { all_evaluations_passed?: boolean }
    | undefined;

  return gate?.all_evaluations_passed !== false;
}
...
export async function upsertLessonProgress(
  ...
  if (existingRows.length > 0) {
    const existing = existingRows[0] as DatabaseRow;

    // Optimistic concurrency: reject if client holds a stale version.
    if (clientVersion !== undefined && existing.version !== clientVersion) {
      return { conflict: true as const, current_version: existing.version as number };
    }

    // completed-wins policy: once a lesson is marked complete server-side,
    // a deferred offline write cannot revert it.
    if (existing.completed === true && completed === false) {
      return ProgressDtoSchema.parse(existing);
    }
